...
**Important:** Unlike the generic `Plugin` interface on the [overview page](/docs/plugins/slots), core slots do not have per-slot props. Each slot renderer receives only the host context — `(ctx) => BaseRenderable` — not `(ctx, props)`. This is because core slots are framework-free renderables that mount directly into the tree rather than being called with JSX props. If you need per-slot props without a framework binding, use `createSlotRegistry` from the overview page directly.
...
A `CoreSlotContribution` is either a plain renderer `(ctx) => BaseRenderable` or a `CoreManagedSlot` with lifecycle hooks.
...
| Field          | Type                      | Required | Description                                                          |
| -------------- | ------------------------- | -------- | -------------------------------------------------------------------- |
| `render`       | `(ctx) => BaseRenderable` | yes      | Creates the renderable node for this slot                            |
